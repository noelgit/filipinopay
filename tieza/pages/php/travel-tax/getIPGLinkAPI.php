<?php 
	...
	$authoizationUrl = IPG_OAUTH_URL."authorize.php?response_type=code&client_id=".$username."&state=xyz";
	...
	if($authorizationVal->Code){
		...
		//  Initiate curl
		$curlToken 	= curl_init(); 
		$tokenUrl = IPG_OAUTH_URL."getToken.php"; 
		...
		if($tokenVal->access_token){
			...
			foreach($transactions[0] as $key => $value){ 
				$dataDoTransaction["transactions['0']['".$key."']"] = $value; 
			 
			}  

			$curlDoTransaction = curl_init(); 

			$urlDoTransaction = IPG_ENTERPRISE_URL."doTransaction.php";
